...
function replaceIteratorKey(chunk, loop, input, debug) {
    ...
    matcher.forEach((entry) => {
        ...
        if (mask.includes(Constants.AST_KEYSELF))
            return outStack.push(mask.replace(Constants.AST_KEYSELF, entry));
...
function render(ctx, data, chunk, debug) {
    var _a, _b;
    ...
    for (const p of partials) {
        const signature = (0, util_1.genInlineScope)(`${Constants.AST_PARTIAL_SIGNATURE}=${p.name}`);
...
function compile(args) {
    var _a, _b;
    const debugConfig = (0, config_1.getDebug)(args.ctx.config.debug);
