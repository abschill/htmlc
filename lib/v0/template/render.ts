...
export default function render( name: string, _varList:Object, inp: string, config: Loader ) {
    let _copy = inp;
    ...
    if( Object.keys( _varList ).length > 0 ) {
        ...
        if( num_iterables === iterators?.length ) {
            const _dom = _copy;
            const _parser = parsable( name, _varList, _dom );
            let outVal = [];
            let outObj = [];
            ...
            console.warn( 'iterators dont match, rendering vals only' );
            console.log( iterators );
            _copy = renderVals( name, _copy, _varList, config.verbose );
        }
    }
    return _copy;
}
