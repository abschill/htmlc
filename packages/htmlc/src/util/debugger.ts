...
const _ = console.log;
export function createDebugger(options: SSROptions | SSGOptions) {
	if (!options.debug) {
		return;
	}
	...
	function log(event_signature: DebugEventSignature, data: object | string) {
		_(color.fg.blue('htmlc::event_signature: '), event_signature);
		_(data);
	}

	function err(event_signature: DebugEventSignature, data: object | string) {
